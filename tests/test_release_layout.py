...
class ReleaseLayoutTests(unittest.TestCase):
    ...
    def readme_marker() -> str:
        """Derive the probe marker from the current README heading."""
    ...
    def test_every_command_hook_has_a_windows_override(self) -> None:
        ...
        for handler in commands:
            ...
            self.assertTrue(handler["command"].startswith("python3 -I -S "), handler["command"])
    ...
    def test_release_checker_ignores_only_ast_proven_python_call_assignments(self) -> None:
        ...
        self.assertFalse(CHECKER._secret_pattern_found(Path("fixture.py"), source, rule_id, pattern))
        self.assertTrue(CHECKER._secret_pattern_found(Path("fixture.txt"), source, rule_id, pattern))
        literal = "B" * 24
        for candidate in (
            f'token = {callable_name}("{literal}")\n',
            f"token = {callable_name}(\n",
            f'password="{literal}"\n',
        ):
            with self.subTest(candidate=candidate):
                self.assertTrue(
                    CHECKER._secret_pattern_found(Path("fixture.py"), candidate, rule_id, pattern)
                )
